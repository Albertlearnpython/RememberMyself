...
- 补全来源统一通过 `provider` 字段切换：`weread / douban`
...
- 让短评、原因、长笔记保持同一思路区块
- 让附件配置独立，不打断正文编辑
- 让长字段改为整行展示，减少 “短字段 + 长 URL” 混排造成的视觉参差
- 让“已有标签 / 新增标签”改为上下堆叠，避免编辑时左右两栏拥挤
...
- `封面图链接` 改为单独整行，便于复制、粘贴和核对
- `附件归档` 的下载开关改为独立动作卡片，不再挤在三列表单里
